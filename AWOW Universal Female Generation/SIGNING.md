...
checksums and GitHub build-provenance attestations identify its release ZIPs;
the project does not create or claim an Authenticode signature for this
version.

## Release authority and provenance

The canonical artifacts are built by `.github/workflows/ufg-release.yml` from a
commit reachable from `main`. Native compilation is pinned by `toolchain.json`,
defined by `toolchain.json`, on GitHub's `windows-2022` runner, MSVC
...
authority. Local provenance is labeled `local_smoke_test`; only the artifacts
