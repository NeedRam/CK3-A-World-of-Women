...
if (-not $SkipElevationCheck -and [Environment]::OSVersion.Platform -eq [PlatformID]::Win32NT) {
    ...
    if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        ...
        if ($WriteFaultAt) { $arguments += @('-WriteFaultAt', ('"{0}"' -f $WriteFaultAt)) }
        try {
...
& (Join-Path $PSScriptRoot 'powershell\engine.ps1') -Operation uninstall -TargetRoot $TargetRoot -PackageRoot $PackageRoot -Confirmation $Confirmation -Interactive:$Interactive -Json:$Json -WriteFaultAt $WriteFaultAt
exit $LASTEXITCODE
